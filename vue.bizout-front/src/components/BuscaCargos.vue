<template>
    ...
    <div v-else class="mb-2 flex-grow overflow-y-auto space-y-2 p-2 bg-gray-50 rounded-md max-h-[300px] mt-4">
        <template v-if="resultadoCarregado && resultados.length">
            <!-- Resultados -->
            <label v-for="cargo in resultados" :key="cargo.id"
                class="flex items-center gap-2 text-sm text-gray-700 hover:bg-gray-100 px-2 py-1 rounded-md cursor-pointer">
                <input type="checkbox" :value="cargo" v-model="cargosSelecionados" class="accent-blue-600 w-4 h-4" />
    ...
    <div class="flex justify-start items-center mt-auto pt-0 space-x-3">
        <button v-if="resultadoCarregado && resultados.length" @click="selecionarTodosCargos"
        ...
        <button v-if="resultadoCarregado && resultados.length" @click="limparTodosCargos"
        ...
        <span v-if="resultadoCarregado && resultados.length" class="text-xs text-gray-500 ml-2">
            Selecionados: {{ cargosSelecionados.length }}
        </span>
...
const cargosSelecionados = ref([...props.modelValue])

// Sincroniza ida e volta com o pai
watch(cargosSelecionados, (val) => {
    emit('update:modelValue', val)
})

const cargosPopulares = [
    { id: 1, nome: 'INSS' },
...
watch(termo, async (valor) => {
    ...
    try {
        const res = await fetch(`https://elastic.bizout.com.br/crg?srch=${encodeURIComponent(valor)}`, {
            signal: controller.signal
        })

        const data = await res.json()
        resultados.value = Object.entries(data).map(([id, nome]) => ({
            id,
            nome
        }))
    } catch (e) {
...
function selecionarcargo(cargo) {
    if (cargosSelecionados.value.length > 90) {
        window.mostrarPopup('Atenção', 'Não é possível selecionar mais cargos.', 'warning', 7000)
        return
    }
    if (!cargosSelecionados.value.some((o) => o.id === cargo.id)) {
        cargosSelecionados.value.push(cargo)
    }
}

function selecionarTodosCargos() {
    if (resultados.value.length > 90) {
        window.mostrarPopup('Atenção', 'Muitos resultados. Refine sua busca.', 'warning', 7000)
        return
    }

    // Adiciona apenas os cargos que ainda não estão selecionados
    const novos = resultados.value.filter(
        (r) => !cargosSelecionados.value.some((c) => c.id === r.id)
    )
    cargosSelecionados.value = [...cargosSelecionados.value, ...novos]
}

function limparTodosCargos() {
    cargosSelecionados.value = []
}
