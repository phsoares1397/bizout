<template>
    ...
    <div v-else class="mb-2 flex-grow overflow-y-auto space-y-2 p-2 bg-gray-50 rounded-md max-h-[300px] mt-4">
        <template v-if="resultadoCarregado && resultados.length">
            <label v-for="orgao in resultados" :key="orgao.id"
                class="flex items-center gap-2 text-sm text-gray-700 hover:bg-gray-100 px-2 py-1 rounded-md cursor-pointer">
                <input type="checkbox" :value="orgao" v-model="orgaosSelecionados" class="accent-blue-600 w-4 h-4" />
...
const orgaosSelecionados = ref([...props.modelValue])

// Sincroniza ida e volta com o pai
watch(orgaosSelecionados, (val) => {
    emit('update:modelValue', val)
})

// Lista de órgãos populares
const orgaosPopulares = [
    { id: 1, nome: 'INSS' },
...
// Busca por órgãos conforme o texto digitado
watch(termo, async (valor) => {
    ...
    // Aborta busca anterior se necessário
    if (controller) controller.abort()
    controller = new AbortController()

    try {
        const res = await fetch(`https://elastic.bizout.com.br/org?srch=${encodeURIComponent(valor)}`, {
        ...
        if (e.name !== 'AbortError') {
            console.error('Erro ao buscar órgãos:', e)
        }
        resultados.value = []
    } finally {
...
// Adiciona órgão popular ao selecionado
function selecionarOrgao(orgao) {
    if (!orgaosSelecionados.value.some((o) => o.id === orgao.id)) {
        orgaosSelecionados.value.push(orgao)
    }
}
</script>