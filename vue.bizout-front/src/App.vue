<template>
  ...
  <div class="min-h-screen mx-auto px-0 lg:px-5">
    <!-- Navbar -->
    <header class="fixed inset-x-0 top-0 z-50 h-[58px] w-full bg-[#414141] shadow flex items-center justify-end px-4">
    ...
    <main class="mt-[8px] pt-[18px] p-1 lg:p-4"> <!-- w-full md:max-w-[90vw] mx-auto -->
      ...
      <div v-else-if="loaded">
        ...
        <div v-else>
          ...
          <!-- Lista de questões -->
          <div :class="isTwoColumns ? 'columns-2 gap-6' : 'columns-1 gap-6'">
            <div v-for="(post, index) in posts" :key="index" class="mb-6 break-inside-avoid relative">
              ...
              <div
                class="flex flex-col items-start gap-2 rounded-lg border border-gray-200 mb-0 bg-gray-50 shadow-sm p-4">
                <!-- Tags / Assuntos -->
                <div class="flex flex-wrap items-center gap-2 text-sm text-gray-700">
                  <a :href="`https://bizout.com.br/qst_id/${post[0]}`"
                    class="hidden md:inline text-sm uppercase tracking-wide text-gray-500 mr-2">
                    C{{ post[0] }}:
                  </a>
                  <span v-for="(assuntos, assuntos_index) in post[3]" :key="assuntos_index"
                ...
                <div class="text-sm text-gray-700">
                  <span class="font-semibold">Ano:</span> {{ post[2][3] }} - {{ post[2][2] }}
              ...
              <p class="text-gray-900 mt-0 p-4 pb-0" v-html="post[7][1]"></p>
              <p v-if="post[7][0]" class="text-gray-900 my-4 mt-0 p-4" v-html="post[7][2]"></p>
              ...
              <div class="flex justify-center w-full">
                <div
                  ...
                  <button
                    ...
                    <!-- Tooltip -->
                    <span
                      class="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-xs rounded px-2 py-1 whitespace-nowrap opacity-0 pointer-events-none transition-opacity duration-200 group-hover:opacity-100">
                      Responder
                    </span>
                  </button>
                  ...
                  <button
                    ...
                    <span :class="isTwoColumns ? 'hidden' : 'hidden sm:inline'">Comentários ({{ post[9] }})</span>
                  ...
                  <button
                    ...
                    <span :class="isTwoColumns ? 'hidden' : 'hidden sm:inline'">Salvar</span>
                  ...
                  <button
                    ...
                    <svg height="16" width="16" viewBox="0 0 24 24" fill="currentColor">
                      ...
                      <path d="M7 12h10M19 6L7 12 19 18" stroke="currentColor" stroke-width="2" />
                    </svg>
                    <span :class="isTwoColumns ? 'hidden' : 'hidden sm:inline'">Compartilhar</span>
                  ...
                  <button
                    ...
                    <span :class="isTwoColumns ? 'hidden' : 'hidden sm:inline'">Reportar Erro</span>
  ...
  <footer class="bg-gray-900 text-gray-200 mt-8">
    <div
      class="max-w-7xl mx-auto px-4 py-8 flex flex-col md:flex-row justify-center items-center space-y-4 md:space-y-0">
...
async function loadPage(pg = 1, set = 0, filtros) {
  ...
  try {
    ...
    const bancasIds = filtros.banca.map(d => parseInt(d.id))
    const orgaosIds = filtros.orgao.map(d => parseInt(d.id))
    const cargosIds = filtros.cargo.map(d => parseInt(d.id))
    ...
    addParam("bnc", bancasIds)
    addParam("org", orgaosIds)
    addParam("crg", cargosIds)
    ...
    const url = `https://questoes.bizout.com.br/v?${params.toString()}`
...
async function marcarQuestaoCorreta(markerId, qst_id, banco) {
  ...
  try {
    const res = await fetch('https://tools.bizout.com.br/mkrs-insert-correct', {
      method: 'POST',
...
async function marcarQuestaoErrada(markerId, qst_id, banco) {
  ...
  try {
    const res = await fetch('https://tools.bizout.com.br/mkrs-insert-wrong', {
      method: 'POST',
      credentials: 'include', // envia cookies (session_id)
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams(data)
